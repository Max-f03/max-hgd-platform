    ...
    >
      ...
      <div className="px-4 py-4 shrink-0">
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 rounded-lg flex items-center justify-center text-white font-bold text-xs shrink-0" style={{ background: "linear-gradient(135deg,#1D4ED8,#2563EB)" }}>
            M
          </div>
          <div className="flex flex-col leading-none">
            <span className="text-xs font-bold" style={{ color: "var(--d-t1)" }}>Max HGD</span>
            <span className="text-[10px] mt-0.5" style={{ color: "var(--d-t4)" }}>Portfolio Admin</span>
          </div>
          <button
            type="button"
            onClick={toggle}
      ...
      <nav className="flex-1 overflow-y-auto py-3 px-2.5">
        {nav.map(({ section, items }) => (
          <div key={section} className="mb-4">
            <p className="px-2 mb-1 text-[9px] font-semibold uppercase tracking-widest" style={{ color: "var(--d-t4)" }}>
              {section}
            </p>
                ...
                return (
                  <li key={item.href}>
                    <Link
                      href={item.href}
                      prefetch={true}
                      className="flex items-center justify-between px-2 py-1.5 rounded-lg text-xs transition-all duration-150"
                      style={{
      ...
      {/* User */}
      <div className="px-2.5 py-2.5 shrink-0 border-t" style={{ borderColor: "var(--d-border)" }}>
        <div className="flex items-center gap-2 px-1.5 py-1.5 rounded-lg">
          <div className="w-6 h-6 rounded-full flex items-center justify-center overflow-hidden text-white text-[9px] font-bold shrink-0" style={{ background: "#1D4ED8" }}>
            {avatarUrl ? <img src={avatarUrl} alt="Avatar" className="h-full w-full object-cover" /> : displayName.slice(0, 2).toUpperCase()}
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-xs font-semibold truncate" style={{ color: "var(--d-t1)" }}>{displayName}</p>
            <p className="text-[10px] truncate" style={{ color: "var(--d-t4)" }}>{displayEmail}</p>
          </div>
          <button
            type="button"
            ...
            className="ml-auto flex h-7 w-7 items-center justify-center rounded-lg text-red-600 hover:bg-red-50 transition-colors"
